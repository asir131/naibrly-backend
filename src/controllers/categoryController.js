...
const initializeDefaultData = async () => {
    try {
        ...
        const categories = [
            ...
            { name: 'More Services', description: 'Additional service categories', order: 3 },
            { name: 'Additional Resources', description: 'Extra resources and services', order: 4 }
        ];
        ...
        const defaultData = [
            {
                ...
                services: ['Plumbing', 'Locksmiths', 'Appliance Repairs', 'Door & Window Repairs', 'HVAC', 'Electrical']
            },
            {
                category: 'Interior',
                type: 'Cleaning & Organization',
                services: ['House Cleaning', 'Carpet Cleaning', 'Upholstery Cleaning', 'Home Organization', 'All Furniture Cleaning', 'Junk Removal', 'Duct & Vent Cleaning', 'Pool Cleaning', 'Commercial Cleaners']
            },
            {
                category: 'Interior',
                type: 'Renovations & Upgrades',
                services: ['General Contracting', 'Carpenters', 'Bathroom Remodeling', 'Kitchen Remodeling', 'Flooring Installation', 'Carpet Installation', 'Basement Remodeling']
            },
            {
                category: 'Exterior',
                type: 'Exterior Home Care',
                services: ['Roofing', 'Window Washing', 'Chimney Sweeps', 'Gutter Cleaning', 'Deck Contractors', 'Siding', 'Concrete & Masonry']
            },
            {
                category: 'Exterior',
                type: 'Landscaping & Outdoor Services',
                services: ['Lawn Care', 'Landscaping Design', 'Gardening', 'Sprinkler System Repairs', 'Artificial Turf Installation', 'Stump Grinding', 'Sod Installation', 'Arborists']
            },
            {
                category: 'More Services',
                type: 'Moving',
                services: ['Local Movers', 'Long Distance Movers', 'Piano Movers', 'Packing & Unpacking', 'Move In & Move Out Cleaning', 'Storage Companies', 'Furniture Movers']
            },
            {
                category: 'More Services',
                type: 'Installation & Assembly',
                services: ['Holiday Light Hanging', 'TV Mounting', 'Security Camera Installation', 'Appliance Installation', 'Ceiling Fan Installation', 'Generator Installation', 'Furniture Assembly']
            }
        ];
        ...
        for (const data of defaultData) {
            ...
            console.log(`✅ Services added to ${data.type}: ${data.services.length} services`);
        }

        console.log('🎉 Default data initialization completed!');
        console.log(`📊 Summary:`);
        console.log(`   - Categories: ${Object.keys(createdCategories).length}`);
        console.log(`   - Category Types: ${totalTypesCreated}`);
        console.log(`   - Services: ${totalServicesCreated}`);
...
const initializeDefaults = async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: 'Default categories, types, and services initialized successfully'
        });
    } catch (error) {
...
const createCategoryTypeWithServices = async (req, res) => {
    try {
        ...
        // Validate category
        const allowedCategories = ['Interior', 'Exterior', 'More Services', 'Additional Resources'];
