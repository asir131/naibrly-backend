...
exports.initializeAdmin = async () => {
    try {
        const existingAdmin = await Admin.findOne({ email: process.env.ADMIN_USERNAME });
...
exports.getDashboardStats = async (req, res) => {
    try {
        ...
        const pendingApprovals = await ServiceProvider.countDocuments({ isApproved: false });
        const activeProviders = await ServiceProvider.countDocuments({ isApproved: true, isActive: true });
...
exports.approveProvider = async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: `Service provider ${approved ? 'approved' : 'rejected'} successfully`,
            data: { provider }
        });
